<template>
	<div id="float" class="wrapper">
		<div class="search-bar flex">
			...
			<div class="seach-bar-item flex flex-sc">
				<button class="search-btn" @click="openFloatSettingAlert(1)">新增</button>
			</div>
		</div>
		...
		<float-setting v-if="floatSettingAlert.state" v-on:submitSuccess="getList"></float-setting>
	</div>
</template>

<script>
	import {Util} from '../../lib/util.js';
	import floatSetting from '../../components/floatSetting.vue';
	...
	export default {
		data() {
			...
			computed: {
			    floatSettingAlert(){
			    	return this.$store.state.floatSettingAlert
			    },
			},
			components: {
				'float-setting': floatSetting,
			},
			methods: {
				search(){
					this.pageNum = 1;
				...
				allClick(e){//事件委托到tbody上
					...
					switch(classStr){
						case 'mode1'://编辑
							...
							this.openFloatSettingAlert(2,message);
							break;
					}
				},
				getList(){//获取列表
					this.loading = true;
					this.api.getN('infoActivityFloatController.do?getFloat',{
				...
				openFloatSettingAlert(type1,message){//打开编辑弹窗
					if(message){
						this.$store.commit('pushFloatSettingMessage',message);
					};
